...
def forward_modeling_single_shot(record, table, par_files):
    ...
    if checkpointing:
        ...
        # time_range = TimeAxis(start=0, stop=(num_samples-1)*samp_int, step=8)
        ...
        dcal, u, v = solver_tti.forward()[0:-1]
        print(dcal.data.shape)
        print(retrieved_shot.shape)
        dcal_resam = dcal.resample(num=num_samples)
        form = 'shot_{}.bin'.format(str(record).zfill(5))
        g = open(form, 'wb')
        np.transpose(dcal_resam.data).astype('float32').tofile(g)
...
def get_slurm_dask_client(n_workers, n_cores, n_processes):
    ...
    cluster = SLURMCluster(cores=n_cores,
                           ...
                           interface='ib0',
                           queue='standard',
                           job_extra=['-e slurm-%j.%N.err', '-o slurm-%j.%N.out',
...
if __name__ == "__main__":
    print('start')
    # Start Dask cluster
    cluster = LocalCluster(n_workers=4, threads_per_worker=2, death_timeout=600)
    c = Client(cluster)
